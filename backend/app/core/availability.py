...
logger.info(f"🔧 Environment variables loaded: {debug_vars}")
...
class Settings(BaseSettings):
    ...
    # ... (all properties from your backup)
    # (Omitted here for brevity, but will be included in the actual file)
...
# Debug: Log what settings were actually loaded
logger.info("🔧 Settings instance created. Checking loaded values:")
logger.info(f"  SECRET_KEY: {'✅ LOADED' if settings.secret_key else '❌ NOT LOADED'}")
logger.info(f"  ZERODHA_API_KEY: {'✅ LOADED' if settings.zerodha_api_key else '❌ NOT LOADED'}")
logger.info(f"  TELEGRAM_BOT_TOKEN: {'✅ LOADED' if settings.telegram_bot_token else '❌ NOT LOADED'}")
logger.info(f"  NEWS_API_KEY: {'✅ LOADED' if settings.news_api_key else '❌ NOT LOADED'}")
logger.info(f"  ALPHA_VANTAGE_API_KEY: {'✅ LOADED' if settings.alpha_vantage_api_key else '❌ NOT LOADED'}")

# CONFIG_AVAILABLE: True if .env loaded and required fields are present
CONFIG_AVAILABLE = bool(settings.secret_key)

# === Feature Availability Flags (added for system boot) ===
...
INTERACTIVE_TRADING_AVAILABLE = True
NEWS_SIGNAL_INTEGRATION_AVAILABLE = True 